...
| `--proxy-bypass` | open | none | all | Add an explicit target bypass rule. Repeat or comma-separate exact DNS, leading-dot suffix, IP, CIDR, or port-qualified authority rules. Ambient proxy variables are never inherited. |
