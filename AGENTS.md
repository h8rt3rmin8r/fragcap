...
S143 closes #400 at the confirmation-gated Steam client-authoring boundary. Only a durable target with an absent launch field, one positive canonical Steam anchor, and one fresh exact application and install-root discovery candidate can produce the separate plan. The appinfo executable remains proposal evidence until the operator positively asserts that it holds the sockets. The plan binds the complete target, candidate, conserved discovery, effective store, proposed and resulting values, and a no-effect list. After input, fresh authority must reproduce byte-for-byte before an immediate complete-row conditional transaction fills only the client declaration and authored fidelity. Every present value, drift, race, and non-positive response remains unchanged. Registration, setup, and session authorization stay separate. S143 adds no dependency, package, schema, non-Steam topology authoring, internal multi-attempt loop, resume, or completion claim, and #380 remains open.
