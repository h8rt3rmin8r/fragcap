...
S142 (`specs/142-guided-target-registration`) follows S141 and closes child issue #398 at the confirmation-gated registration boundary. `fragcap calibrate` preserves stored selection precedence, falls back to the existing bounded discovery composition only on a clean miss, and offers only one exact Steam application identifier or Unicode-aware case-insensitive display-name match. One complete domain-separated plan binds the candidate, conserved discovery account and warnings, and effective store before it is flushed for interactive default-no or exact structured confirmation. Discovery is repeated before the unchanged authority reaches the shared registration operation. The resulting durable target enters S139-S141 without promoting discovery hints into launch topology, so a new row truthfully retains the existing missing-declaration limitation. S142 adds no dependency, package, schema, automatic bulk registration, internal multi-attempt loop, resume, or completion claim, and #380 remains open.
