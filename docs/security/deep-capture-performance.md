...
Reports are JSON Lines with monotonic sequence values, a registry digest, exact row identities, and a terminal completeness decision. They contain no credentials, capability material, private keys, payload contents, usernames, home paths, or machine identifiers. Raw timing values are comparable only within the recorded operating-system family, architecture, build profile, campaign profile, and registry digest. Two fresh Windows campaigns must agree on every outcome; raw medians use a 75-percent diagnostic tolerance because subsecond cross-process scheduling is noisy. Resource and conservation failures remain comparable across hosts.
