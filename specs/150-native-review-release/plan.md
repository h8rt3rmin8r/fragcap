...
- D-4: Commit validated documentation preparation before Cargo release requires a clean tree. Preview the version operation, perform the local version-only bump on the release branch, update exact version assertions and regenerated goldens, assemble all changelog fragments with `cargo xtask changelog --release`, write the one-screen notes, then run the full gate again. Cargo release does not tag/push/publish under the existing configuration.
