...
The current real-target path is deliberately narrow. Ordinary Deep Capture requires current `proxy-routing = reached-client` evidence for `steam-protocol-cold`. Unknown, stale, conflicting, or wrong-launch evidence is refused. Compatibility calibration can produce initial local evidence through a trust-free reachability phase followed, only after current final-client routing, by a separate TLS phase. Routing does not itself prove environment propagation. An already-running Steam process is a warm launch fragcap does not own, and direct-executable managed launch is not implemented, so both are refused before session side effects. The [Deep Capture compatibility reference](/docs/reference/deep-capture-compatibility) carries the complete current traffic and launch matrix.
