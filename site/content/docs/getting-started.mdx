...
- **mitmdump is required only for Deep Capture.** Install the Windows mitmproxy package so `mitmdump` is on `PATH`. Do not configure a system-wide proxy or install mitmproxy's CA for this guide; fragcap prepares its target-scoped proxy session and manages its own purpose-specific CA after explicit confirmation.
...
Capture observes packets on the selected interfaces and attributes matching flows to the target process when the available socket and process evidence permits it. Unattributed packets remain present and marked. Payload bytes are retained by default; `--no-payload` is an explicit choice to retain metadata without payload bytes. Excluded or discarded observations are accounted for in the completion statistics.
...
`targets show` is read-only. It does not start a proxy or target, change trust, contact a catalog, or refresh evidence. The v0.7.0 real-target path requires current facts proving that scoped proxy configuration both propagated through a cold Steam protocol launch and reached the final client:

```text
compatibility:
  proxy-routing = reached-client | launch=steam-protocol-cold | source=observed-run | freshness=current
  proxy-propagation = confirmed | launch=steam-protocol-cold | source=observed-run | freshness=current
```

Proceed only when both facts are current for `steam-protocol-cold`. Other rows, including protocol behavior, CA acceptance, and inspectability, describe observations without replacing those two routing requirements.

<Callout type="warn" title="Unknown is a stop, not a guess">
If the detail view reports `unknown`, stale or conflicting routing facts, or evidence for another launch case, stop here. v0.7.0 has no supported end-user workflow that creates the initial compatibility evidence for an unknown target. Do not infer compatibility from the title, engine, launcher, executable, or another target. See the [Deep Capture compatibility reference](/docs/reference/deep-capture-compatibility).
</Callout>

The supported real-target launch case also requires Steam to be closed before the session begins. An already-running Steam process is a warm launch that fragcap does not own, so Deep Capture refuses it. Direct-executable managed launch is also not implemented in v0.7.0.

## 7. Run a known-compatible Deep Capture

Review the planned side effects before continuing: fragcap starts a loopback mitmdump child, starts ordinary packet Capture, prepares target-scoped proxy environment for the managed launch, may trust a purpose-specific CA in the current-user Root store after your explicit confirmation, writes a session bundle, and attempts to remove session-owned trust and processes during cleanup. It never falls back to a system-wide proxy.
...
`--trust-ca` confirms that this run may make the stated current-user trust change when HTTPS inspection needs it. Omit `--har` or `--key-log` when you do not need those sensitive optional artifacts. Do not add `--yes` on a first run; read the interactive confirmation and planned side effects.
...
| Traffic | What v0.7.0 can add beyond Capture |
| --- | --- |
...
| `http.har` | Optional [HAR](/docs/glossary/file-and-wire-formats#har) projection of observed HTTP semantics. It cannot contain headers or bodies that v0.7.0 never retained. |
