...
```text
fragcap calibrate (<SELECTOR> | --target <SELECTOR> | --id <ID>) [--protocol <PROTOCOL>]... [OPTIONS]
```
...
Guided calibration tests additionally freeze repeatable candidate normalization, reachability-first ordering, deterministic multi-session execution, separate plan responses, exact case non-repetition, the closed fourteen-attempt bound, distinct bundle destinations, final-client candidate eligibility, launcher and unknown exclusion, strongest same-session inspectability selection, exact fact reassessment before progression, every stop boundary, warm preservation, structured attempt and coverage fields, and parseable durable continuation commands. Controlled reachability and TLS runs must prove that a requested protocol remains distinct from observed traffic and from the current exact fact that alone suppresses future work.
