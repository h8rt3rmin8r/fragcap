...
S134 (`specs/134-plan-bound-authorization`) follows S133 and closes #382 at the complete Deep Capture authorization boundary. One canonical plan binds the exact target launch authority, process-local session CA identity, loopback family and routing policy, requested artifacts, deadlines, possible fact writes, cleanup obligations, and refusal boundaries before any session effect. Interactive execution confirms that plan once. Structured execution returns its versioned BLAKE3 identifier through `--authorize-stdin` in the same process. Hidden legacy Deep Capture `--trust-ca` and `--yes` inputs only return migration guidance for one release. S134 adds direct dependency edges for `blake3` and `subtle`, both already in the locked workspace graph, and adds no package or schema change.
