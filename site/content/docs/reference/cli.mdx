...
Older releases and their preserved changelog pages may refer to command forms that the current CLI no longer accepts. Use these replacements with the published v0.10.2 command surface:
...
`--roles` is enforced. `--direction in` and `--direction out` are recorded and reported, but v0.10.2 still writes both directions.
