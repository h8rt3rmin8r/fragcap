...
S141 extends that front door through one plan-bound protocol attempt per invocation. Repeatable candidates remain measurement requests, automatic continuations admit only concrete final-client observations, exact facts are re-read before coverage is reported, and unresolved candidates travel in the next command rather than workflow storage. S142 adds stored-first, confirmation-gated registration for one exact discovered Steam identifier or Unicode-aware case-insensitive display name. The complete candidate, conserved discovery account and warnings, and store are plan-bound; discovery is repeated with exact changed-selection diagnostics before the shared registration operation, and the durable stable identifier replaces the discovery selector throughout existing guidance without promoting hints into launch topology. S143 adds a separate confirmation-gated Steam client plan for a truly absent launch value, binding exact application and install-root discovery plus one whitespace-free appinfo executable proposal before an operator socket-holder assertion reaches a complete-row conditional update. Ineligible authority is reported by exact condition, every emitted plan receives a terminal outcome across target re-read, persistence, and post-update verification, and present declarations and stale authority remain unchanged. Internal multi-attempt execution, resume, persistence, and completion remain in #380.
