...
These instructions describe guided calibration and native functionality shipped in v0.10.0 on 2026-09-15. Follow documentation matching your installed bytes; v0.10.2 includes the complete S154 through S158 native documentation, review-intake and deterministic certification engineering. The [Native product contract](/docs/reference/native-documentation) records the exact supported boundary. Optional field observations do not gate implementation completion; file an observed failure against the affected active release.
</Callout>
