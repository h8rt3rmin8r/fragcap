...
S160 supersedes S158's capacity conclusion after merged-main Windows evidence showed that an already-ready writer can be descheduled while the finite QUIC burst is produced. Product sessions and the canonical performance harness now share a 16,384-event application queue with an independent 32 MiB retained-payload bound while forwarding remains independent and admission remains nonblocking. Deterministic post-readiness stalls fill each exact bound without loss, prove the next event is refused and counted, then prove ordered drain and zero terminal ownership. Performance report schema 2 requires exact application event attempts and queue capacity, refuses a canonical burst that exceeds the capacity even if concurrent draining kept the observed peak low, and partitions writer-observed payload into successfully retained, intentionally omitted, and retained-storage-dropped bytes before adding pre-writer queue loss to the total. Queue-loss authorities use original observed lengths even when retained streaming payload is omitted or truncated; storage-loss authorities use the retained bytes that actually failed persistence so one event cannot be counted as both retained and lost. Historical loss-free schema 1 evidence remains readable, unknown versions remain refused, the registry remains schema 1, and first-attempt Windows acceptance remains mandatory.
