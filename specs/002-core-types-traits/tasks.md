...
- [ ] T001 Add `bytes = "1.12"` to `[workspace.dependencies]` in `Cargo.toml`,
  per plan D-1
- [ ] T002 Add `bytes.workspace = true` to `[dependencies]` in
  `crates/fragcap-core/Cargo.toml`
- [ ] T003 Run `cargo build --workspace --locked` and commit the resulting
...
- [ ] T004 [P] Define `Timestamp` in `crates/fragcap-core/src/packet.rs` as an
...
- [ ] T011 [US1] Define `Endpoint` in `crates/fragcap-core/src/flow.rs`, an
  address, port, and protocol
- [ ] T012 [US1] Define `Attribution` in
  `crates/fragcap-core/src/attribution.rs` with `pid`, `process` and `role` as
  shared strings, and `stage`, per FR-007
- [ ] T013 [US1] Define `RawPacket` in `crates/fragcap-core/src/packet.rs` with
  `ts`, `data` as the payload alias, and `orig_len` separate from payload
  length, per FR-008
- [ ] T014 [US1] Define `CapturedPacket` in `crates/fragcap-core/src/packet.rs`
  with the raw fields plus optional `flow`, `direction`, and `attribution`, per
  FR-009
- [ ] T015 [US1] Define the three error enums in
...
- [ ] T016 [US1] Declare `PacketSource`, `FlowAttributor`, `ProcessWatcher`,
...
- [ ] T017 [US1] Document every public item, naming the owning slice for each
...
- [ ] T018 [US2] Define `FlowKey` in `crates/fragcap-core/src/flow.rs` with
  `proto`, `local`, and `remote`, documenting that `local` is always the
  capturing host's endpoint, per FR-002
- [ ] T019 [US2] Define `AttributionKey` in `crates/fragcap-core/src/flow.rs`
  with a `Pair` variant and a `Local` variant, and deliberately no variant
  carrying a remote for UDP, per FR-004
- [ ] T020 [US2] Implement `FlowKey::attribution_key` returning `Pair` for TCP
  and `Local` for UDP, per FR-003
- [ ] T021 [US2] Implement wildcard matching for a UDP local endpoint so both a
...
- [ ] T022 [US3] Define `SourceStats` in `crates/fragcap-core/src/stats.rs` with
  `received`, `kernel_dropped`, and `interface_dropped`, using the names from
  specification section 12.4
- [ ] T023 [US3] Define `CaptureStats` in `crates/fragcap-core/src/stats.rs`
...
- [ ] T024 [US3] Implement totals as methods on `CaptureStats`, never as stored
  fields, per FR-025
- [ ] T025 [US3] Implement `CapturedPacket::attribution_state` returning the
...
- [ ] T026 [P] Test V-1 in `crates/fragcap-core/src/flow.rs`: `FlowKey` and
  `AttributionKey` work as map keys, and equal keys hash equally
- [ ] T027 [P] Test V-3 in `crates/fragcap-core/src/flow.rs`: a TCP flow key
  derives `Pair`, a UDP flow key derives `Local`, asserted independently
- [ ] T028 [P] Test V-10 in `crates/fragcap-core/src/flow.rs`: a UDP local
  endpoint matches both a wildcard bind address and a specific address
- [ ] T029 [P] Test V-4 in `crates/fragcap-core/src/attribution.rs`: cloning an
  `Attribution` shares the process name rather than allocating
- [ ] T030 [P] Test V-5 in `crates/fragcap-core/src/packet.rs`: a truncated
  packet reports its original on-wire length
- [ ] T031 [P] Test V-9 in `crates/fragcap-core/src/packet.rs`: each of the
  three attribution states is constructed and read back correctly
- [ ] T032 [P] Test V-6 in `crates/fragcap-core/src/stats.rs`: a total is
  computed from named counters, and an individual cause is assertable alone
- [ ] T033 [P] Test the timestamp is lossless across construction and
  read-back, and that no rounding occurs, supporting FR-011
- [ ] T034 Audit the public surface for P-9 compliance and record the result in
...
- [ ] T035 Test V-7 in `crates/fragcap-core/src/traits.rs`: construct each of
  the four behavioral traits behind a `Box<dyn _>`, which fails to compile if a
  trait stops being dyn-compatible
- [ ] T036 Test V-8 and V-2 in `crates/fragcap-core/src/traits.rs`: wire stub
...
- [ ] T037 Add a `docs/glossary.md` entry for every term this slice introduces,
  per FR-030 and P-6, following the entry template in specification section 4.3.
  Satisfies SC-008
- [ ] T038 Queue the plan D-7 deviation for promotion to specification section
  ...
  does not satisfy
- [ ] T039 Add a `changelog.d/S02-core-types-traits.added.md` fragment
- [ ] T040 Update the `crates/fragcap-core/README.md` status section, since its
...
- [ ] T041 Run `cargo xtask ci` and confirm `ci: all checks passed`. Satisfies
  SC-009
- [ ] T042 Run `cargo xtask neutral` and confirm `fragcap-core` builds for a
...
- [ ] T043 Run `cargo xtask msrv` and read the output, confirming the declared
...
- [ ] T045 Verify every requirement FR-001 through FR-031 and every success
