...
Npcap and runs only where it is installed. Npcap is never bundled or redistributed. After explicit interactive confirmation, `fragcap doctor --fix` may fetch and launch the vendor's signed installer from the official location.
