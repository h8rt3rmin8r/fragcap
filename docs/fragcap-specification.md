...
Guided calibration tests additionally freeze repeatable candidate normalization, content-derived candidate identity, evidence-order independence, explicit ambiguous target and Steam selection, duplicate and stale refusal, reachability-first ordering, deterministic multi-session execution, separate plan responses, exact case non-repetition across processes, the closed fourteen-attempt bound, distinct bundle destinations, final-client candidate eligibility, launcher and unknown exclusion, strongest same-session inspectability selection, exact fact reassessment before progression, every stop boundary, warm preservation, structured attempt and coverage fields, and parseable durable continuation commands. Store tests cover version 10 and version 11 migration, checkpoint and exact-case intent round trip, closed vocabulary and canonical-set refusal, bounded exact attempt history, target cascade, revision races, and unsupported record versions. Controlled command tests cover fresh identity, second-process resume, current-fact suppression, completed no-effect resume, in-flight interruption, target drift, every explicit pause, durable bundle ordinals, distinct fresh plans, IPv6 propagation, unsupported routing, and launch assertion mismatch. Controlled reachability and TLS runs must prove that a requested protocol remains distinct from observed traffic and from the current exact fact that alone suppresses future work.
