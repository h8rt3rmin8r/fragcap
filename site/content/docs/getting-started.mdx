...
Answer the one prompt only after reviewing the exact plan identifier and every effect. Omit `--har` or `--key-log` when you do not need those sensitive optional artifacts. Automation and JSON consumers use `--authorize-stdin`, read the emitted plan event, and return its exact identifier plus one LF without restarting the process. The former Deep Capture `--trust-ca` and `--yes` booleans no longer authorize a run.
