...
| Option | Values | Default | Availability | Meaning |
| --- | --- | --- | --- | --- |
| `--authorize-stdin` | open | `false` | all | Read the exact emitted authorization plan identifier from standard input for the selected attempt. Conflicts with `--restart-warm`. |
...
`calibrate` does not register targets, perform multiple attempts internally, persist workflow state, or bypass Deep Capture plan authorization. A request, an observation, and a current exact compatibility fact remain separate. Ambiguous, invalid, and unavailable proposals report typed limitations before effects and do not invent a next command.
