...
Review the complete plan before continuing. It names the target launch authority, loopback and routing scope, exact session CA identity and current-user trust action, bundle and sensitive artifacts, deadlines, possible fact writes, cleanup obligations, and refusal boundaries. fragcap applies no listener, bundle, trust, launch, Capture, or fact effect until that exact plan is authorized. It never falls back to a system-wide proxy.
