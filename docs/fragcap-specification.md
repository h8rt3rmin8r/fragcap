...
| Q-10 | Which native Rust proxy backend should Deep Capture use? | Build candidate spikes and compare protocol coverage, Windows behavior, licenses, dependency graph, MSRV impact, maintenance posture, and integration cost | #214, #253 | **Open, narrowed 2026-08-29.** The `hudsucker 0.23.0` spike passed the functional proofs but failed the advisory-clean Rust 1.82 graph requirement. Evaluate only the smaller `http-mitm-proxy 0.18.0` fallback next; external `mitmdump` remains shipped meanwhile. |
