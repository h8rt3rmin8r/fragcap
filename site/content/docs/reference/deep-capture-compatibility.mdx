...
First close the selected target and, for a Steam launch, close Steam. Managed
direct, cold Steam platform, and declared publisher-launcher chains are
supported. A warm target enters an explicit operator-confirmed close-and-retry
workflow; fragcap observes the transition but never signals or terminates the
process. Then measure scoped proxy reachability without changing certificate
trust. This example uses the Steam launch case:
