...
These instructions describe the v0.9.0 native path, including managed launch,
scoped IPv4 and IPv6 routing, supported application protocols, bounded evidence,
recovery, and Windows integration. The final Deep Capture completion gate remains
tracked under [#278](https://github.com/h8rt3rmin8r/fragcap/issues/278).
</Callout>
...
flowchart TD
  ...
  proxy["fragcap-proxy: native Deep Capture proxy"]
...
- **The Deep Capture proxy ships with fragcap.** Do not configure a system-wide proxy. fragcap prepares its target-scoped native proxy and exact purpose-specific CA identity for the complete authorization plan, then applies only the reviewed effects.
...
Deep Capture
  proxy backend          ok    fragcap-native 0.9.0
...
Ready targets appear first under `Ready to capture`; targets whose launch chain is unresolved follow under `Needs setup`. Empty groups are omitted, handles are sorted inside each group, and row numbers continue across the complete listing. A row number is an ephemeral selector for the listing you just saw. The handle, such as `sample-target`, is stable for later human commands. `CAPTURE` reports whether fragcap has enough target information to start the passive path. `ENGINE` and `SENSITIVITIES` report detected products or an explicit scan-coverage state; neither column is a compatibility verdict.
...
```powershell
fragcap deep-capture sample-target --launch --calibrate tls --calibration-protocol https --launch-case steam-protocol-cold
```
...
For a Steam target, Steam must be closed before the session begins. An already-running Steam process is a warm launch that fragcap does not own, so Deep Capture refuses it. v0.9.0 also supports eligible cold publisher chains and direct-executable launches from prevalidated paths; an already-running target remains a side-effect-free refusal.

## 7. Run a known-compatible Deep Capture

Review the complete plan before continuing. It names the fully resolved target profile and managed launch authority, loopback and routing scope, exact session CA identity and current-user trust action, bundle and sensitive artifacts, millisecond deadlines, possible fact writes, cleanup obligations, and refusal boundaries. fragcap applies no listener, bundle, trust, launch, Capture, or fact effect until that exact plan is authorized. It never falls back to a system-wide proxy.
...
Answer the one prompt with a complete line only after reviewing the exact plan identifier and every effect. Omit `--har` or `--key-log` when you do not need those sensitive optional artifacts. Automation and JSON consumers use `--authorize-stdin`, read the emitted plan event, and return its exact identifier plus one LF without restarting the process. If prior-session recovery is pending, fragcap refuses before showing a new plan and directs you to `fragcap doctor --fix`. A plan also refuses if its prepared CA expires or its target authority changes while it is reviewed. The former Deep Capture `--trust-ca` and `--yes` booleans no longer authorize a run.
...
| Traffic | What v0.9.0 can add beyond Capture |
| --- | --- |
| HTTP/1.1 and HTTP/2 | Protocol-faithful metadata, bounded raw bodies, supported content-decoding derivations, streaming timing, and explicit loss. |
| HTTPS | The corresponding HTTP evidence across separately verified client-facing and upstream TLS boundaries. Certificate-pinned traffic is refused without downgrade. |
| HTTP/3 and QUIC | Scoped, authenticated HTTP/3 request, response, stream, datagram, timing, refusal, and loss evidence for exact `h3` routes. |
| WebSocket | Verified HTTP/1.1 and HTTP/2 upgrades, frames, bounded messages, compression outcomes, and terminals. |
| SSE and gRPC | Incremental SSE events and bounded gRPC envelopes and status without inventing protobuf meaning. |
| Generic TCP and non-HTTP TLS | Bounded directional plaintext or opaque encrypted chunks with exact terminal and omission evidence. |
| Generic UDP | Metadata and bounded retained payload prefixes for authenticated scoped associations, independently from forwarding. |
...
| `http.har` | Optional [HAR](/docs/glossary/file-and-wire-formats#har) projection of complete observed HTTP semantics. Missing or partial facts remain explicit rather than becoming placeholders. |
...
If it names stale CA trust, proxy state, or an unfinished manifest, review the path and then run:
...
Ordinary MSI uninstall removes the program and its installer-owned machine effects while preserving your targets, writable catalog, profiles, captures, logs, settings, and Deep Capture bundles.

If you deliberately want a clean account, select the unchecked **Remove all fragcap application and user data for my account for a fresh start** option during interactive removal. The dialog shows the exact `%APPDATA%\fragcap` and `%LOCALAPPDATA%\fragcap` roots. The operation is irreversible. fragcap reconciles exact recorded Deep Capture obligations first, then deletes accepted entries without following links or reparse points. Custom paths, exports, Npcap, Wireshark configuration, and independently managed extcap registrations remain untouched.

Silent current-user removal requires both `FRAGCAP_FRESH_START=1` and `FRAGCAP_FRESH_START_SCOPE=current-user`. Administrators handling multiple profiles use a separate preview-bound flow:
...
Review every SID and exact root before the second command. If any profile, root, or entry changed, fragcap refuses and requires a new preview. Another profile's Deep Capture evidence remains because its Windows CurrentUser trust must be reconciled under that profile identity. Run current-user fresh-start or `fragcap doctor --fix` while signed in as the named user, confirm only the exact actions it offers, and retry before calling the account clean.
