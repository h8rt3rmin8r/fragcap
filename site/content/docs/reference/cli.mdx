...
Guide one already registered target toward current Deep Capture routing evidence. The command reads the target, process image inventory, and retained compatibility facts before proposing one next action. Current positive evidence starts no session and points to ordinary Deep Capture. A warm target starts no session unless `--restart-warm` explicitly selects the existing operator-owned close-and-retry workflow. Any selected measurement is limited to one reachability attempt and retains the complete Deep Capture authorization boundary. Generated commands carry the effective local-store path as one PowerShell-quoted argument so their durable identifier resolves against the same store.
...
| Option | Values | Default | Availability | Meaning |
| --- | --- | --- | --- | --- |
| `--authorize-stdin` | open | `false` | all | Read the exact emitted authorization plan identifier from standard input for a selected reachability attempt. Conflicts with `--restart-warm`. |
| `--bundle` | open | none | all | Select the session bundle directory for a selected reachability attempt. |
| `--catalog-db` | open | none | all | Override `FRAGCAP_CATALOG_DB`, then the managed per-user catalog store. |
| `-d`, `--duration` | open | none | all | Bound the selected reachability observation duration. |
| `--id` | open | none | all | Select a stored target by durable identifier. |
| `-i`, `--interface` | open | none | all | Select a capture interface; repeat for more than one. |
| `--local-db` | open | none | all | Override `FRAGCAP_LOCAL_DB`, then the managed per-user local store. |
...
`calibrate` does not register targets, select TLS or protocol calibration, perform multiple attempts, or bypass Deep Capture plan authorization. Ambiguous, invalid, and unavailable proposals report typed limitations before effects and do not invent a next command.
