...
This page describes the public v0.9.0 command tree. Its command headings,
...
that the current CLI no longer accepts. Use these replacements with v0.9.0:
...
reported, but v0.9.0 still writes both directions.
...
Guide one target through current Deep Capture reachability and one protocol attempt per invocation. Stored targets retain precedence. On a clean stored miss, an exact installed Steam application identifier or Unicode-aware case-insensitive display name can produce one complete registration plan. The plan exposes and binds the complete candidate, conserved discovery account and warnings, and effective store before confirmation. Interactive registration defaults to no; structured registration requires the exact emitted `target-registration-v1` identifier through `--authorize-stdin`. Discovery is repeated before the shared single-target registration operation runs. When the resulting exact Steam target has no launch value, a separate `steam-client-setup-v1` plan may present the matching appinfo executable and ask whether it holds the target's sockets. Only one whitespace-free executable path is eligible, and absent, ambiguous, root-mismatched, missing-executable, and unsafe-executable authority is reported distinctly. Only an explicit positive answer reaches a fresh-authority, complete-row conditional update. Every emitted setup plan receives a terminal setup outcome across target re-read, store, and post-update verification failures. Present launch values are never overwritten. Registration, setup, and a later Deep Capture plan remain separate authorizations.

For a target with authoritative topology, the command reads the process image inventory and retained compatibility facts before proposing one next action. Repeatable `--protocol` values request measurements and do not constitute evidence. Missing route evidence selects reachability first. Current route evidence permits one separately authorized protocol attempt. Concrete final-client observations are carried into a new paste-ready command, then current exact facts decide completed and remaining coverage. A warm target starts no session unless `--restart-warm` explicitly selects the existing operator-owned close-and-retry workflow. Generated commands carry the effective local-store path as one PowerShell-quoted argument so their durable identifier resolves against the same store.

```powershell
fragcap calibrate sample-target
fragcap calibrate --id 42 --protocol https
fragcap calibrate --id 42 --restart-warm
```

| Option | Values | Default | Availability | Meaning |
| --- | --- | --- | --- | --- |
| `--authorize-stdin` | open | `false` | all | Read each exact emitted plan identifier from standard input. A discovered target first requires its `target-registration-v1` identifier, an eligible absent Steam client requires its separate `steam-client-setup-v1` identifier, and any later effectful attempt requires its current Deep Capture plan identifier. Conflicts with `--restart-warm`. |
| `--bundle` | open | none | all | Select the session bundle directory for the selected attempt. |
| `--catalog-db` | open | none | all | Override `FRAGCAP_CATALOG_DB`, then the managed per-user catalog store. |
...
| `--protocol` | `http1`, `https`, `http2`, `websocket`, `sse`, `grpc`, `generic-tcp`, `non-http-tls`, `socks5-tcp`, `socks5-udp`, `generic-udp`, `quic`, `http3` | none | all | Request one concrete protocol measurement after reachability. Repeat for more than one candidate; one invocation still runs at most one attempt. |
...
| `--wait` | open | none | all | Set the target acquisition timeout for the selected attempt. |

`calibrate` registers at most one exactly selected, explicitly confirmed discovery candidate through the shared target operation. It does not perform multiple attempts internally, persist workflow state, promote discovery hints into launch authority, or bypass Deep Capture plan authorization. A request, an observation, and a current exact compatibility fact remain separate. Ambiguous, invalid, and unavailable proposals report typed limitations before effects and do not invent a next command.
