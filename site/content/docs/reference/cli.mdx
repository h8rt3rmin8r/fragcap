...
`calibrate` registers at most one exactly selected, explicitly confirmed discovery candidate through the shared target operation. Its multi-attempt state exists only for the current process; it does not persist or resume workflow state, promote discovery hints into launch authority, or bypass per-attempt Deep Capture plan authorization. A request, an observation, and a current exact compatibility fact remain separate. Ambiguous, invalid, and unavailable proposals report typed limitations before effects and do not invent a next command.
