...
```text
fragcap calibrate (<SELECTOR> | --target <SELECTOR> | --id <ID>) [OPTIONS]
```

`calibrate` resolves only an already registered target, obtains one complete query-only process image snapshot, reads retained compatibility facts, and consumes the proposal with child-environment routing and IPv4. It can select at most one exact reachability case. The selected case delegates in-process to the existing `deep-capture --calibrate reachability --calibration-protocol routing` executor, including its complete plan authorization, deadlines, lifecycle, artifacts, terminal exit, and append-only directly observed facts. A successful attempt points back to `fragcap calibrate --id <ID>` so current evidence is reassessed.

Current exact positive routing evidence starts no session and points to `fragcap deep-capture --id <ID> --local-db <EFFECTIVE_PATH> --launch`. A warm target starts no session by default and points to `fragcap calibrate --id <ID> --local-db <EFFECTIVE_PATH> --restart-warm`; that explicit option reuses the existing operator-owned close-and-retry authority before any calibration plan. The post-attempt reevaluation command carries the same store binding. Paths are PowerShell-quoted so whitespace and interpolation characters cannot change the selected store. Post-session completion requires a limitation-free proposal that remains ready for the attempted launch case and selects no further step; changed, malformed, warm, unavailable, or otherwise non-ready launch authority cannot be reported as completed. Ambiguous, invalid, or unavailable proposals report every typed limitation before effects and invent no command. One `calibration.guidance` JSON event carries stable target identity, topology, action, status, launch cases, reason, images, limitations, and nullable next command; human output projects the same facts. S140 does not register targets, select TLS or protocol cases, run multiple attempts, persist workflow state, or weaken ordinary Deep Capture eligibility.
