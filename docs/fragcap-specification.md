...
| 0.1.60-draft | 2026-09-09 | W. Thompson | **Adds deterministic guided calibration case proposals (issue #392).** Extends sections 15, 17.2.1, 25, and 28. One pure stable facade operation derives strict Steam, direct, or publisher topology from a resolved target, requires a complete caller-supplied process image snapshot before declaring a cold case, applies conservative child-environment and IPv4 defaults, and proposes only exact useful reachability or protocol work from retained S121 evidence. Warm, ambiguous, unavailable, invalid, stale, legacy, mismatched, negative, and conflicting states remain explicit. S139 applies no CLI, process, launch, trust, network, artifact, fact, or store effect and leaves parent issue #380 open. |
| 0.1.61-draft | 2026-09-09 | W. Thompson | **Adds the guided reachability calibration front door (issue #394).** Extends sections 15, 17.2.1, 25, and 28. `fragcap calibrate` resolves one existing target, takes one query-only process image snapshot, consumes the S139 proposal, and either delegates one exact reachability case through the existing plan-bound executor or emits ready, warm, or refused guidance without session effects. Stable human and JSON guidance uses durable target identifiers. TLS and protocol selection, automatic registration, multi-attempt execution, and workflow persistence remain in parent issue #380. |
| 0.1.62-draft | 2026-09-10 | W. Thompson | **Adds one guided protocol calibration attempt (issue #396).** Extends sections 15, 17.2.1, 25, and 28. Repeatable concrete `fragcap calibrate --protocol` candidates feed S139 only after current exact reachability. One invocation delegates at most the first plan-bound protocol attempt, derives automatic continuation candidates only from concrete final-client terminal observations, re-reads exact append-only facts, and reports requested, observed, completed, and remaining coverage. Protocol facts now require final-client ownership and collapse same-session inspectability to its strongest directly observed value. Automatic registration, internal multi-attempt execution, workflow persistence, and completion remain in parent issue #380. |
| 0.1.63-draft | 2026-09-10 | W. Thompson | **Adds confirmation-gated guided target registration (issue #398).** Extends sections 15, 17.2.1, 25, and 28. Stored target resolution remains first. Only a clean miss reaches the existing bounded discovery composition, where one exact Steam application identifier or Unicode-aware case-insensitive display name can produce a complete domain-separated registration plan. The plan binds the candidate, conserved discovery account and warnings, and effective store. Interactive input defaults to no, structured input requires the exact plan identifier, and discovery is repeated before the unchanged authority reaches the shared registration operation. The durable result enters existing guidance without promoting discovery hints into launch topology. Internal multi-attempt execution, workflow persistence, resume, and completion remain in parent issue #380. |
| 0.1.64-draft | 2026-09-10 | W. Thompson | **Adds confirmation-gated Steam client setup (issue #400).** Extends sections 15, 17.2.1, 25, and 28. A registered Steam target with an absent launch declaration may join one fresh exact application and install-root discovery candidate, then present its appinfo executable only as a proposal. A complete domain-separated plan binds the target, candidate, conserved discovery, store, proposed client, result, and no-effect boundary. Only an explicit socket-holder assertion can author the client. Fresh revalidation and an immediate complete-row conditional transaction prevent stale or concurrent authority from being overwritten. Present declarations remain untouched, and registration, setup, and any session retain independent authorization. Internal multi-attempt execution, workflow persistence, resume, and completion remain in parent issue #380. |
| 0.1.65-draft | 2026-09-11 | W. Thompson | **Adds bounded guided calibration sequencing (issue #402).** Extends sections 15, 17.2.1, 25, and 28. One `fragcap calibrate` invocation may advance through missing reachability and every current useful requested or eligible observed protocol case. Every attempt re-reads target, process, proposal, and fact authority, uses a distinct bundle, emits a fresh complete S134 plan, and consumes a separate confirmation. Only a fresh current positive fact permits progression. Decline, drift, interruption, failure, partial evidence, warm state, limitations, repetition, and the closed fourteen-attempt bound stop before later effects with exact coverage and continuation. Persistent workflow state, resume, non-Steam topology authoring, and completion remain in parent issue #380. |
...
| 0.1.81-draft | 2026-09-21 | W. Thompson | Corrects the scheduler-dependent QUIC observation loss found after S159 records merged. Updates sections 25.5 and 28.1. Product and performance paths share a finite 16,384-event application queue. A ready-but-stalled consumer regression proves exact-bound admission, nonblocking counted refusal beyond the bound, ordered drain and zero terminal ownership. Performance report schema 2 adds exact producer attempts and capacity, and corrects observed payload totals to include accepted and lost dispositions once. Historical schema 1 remains readable and unknown versions remain refused. |
...
S132 stabilizes the first completion-line Rust integration boundary at `fragcap::deep_capture::api` under the `deep-capture` feature. A reviewed version-one inventory covers session intent, side-effect-free preparation, immutable plans, plan-bound authorization, checked lifecycle control, routing, native proxy selection, observations, classification, calibration, artifacts, recovery, and terminal truth without exporting proxy protocol engines or writer implementations. `SessionConfigBuilder` and `AdapterSetBuilder` are the additive construction paths. The coordinator and injected adapters remain serial and thread-confined, while the cloneable `CancellationToken` is `Send` and `Sync`; cancellation is monotonic, cooperative, checked before effects and between adapter calls, and cannot preempt an in-flight bounded call. Legacy top-level exports remain compatible during the pre-1.0 line but do not create a second stability contract.
...
The application sink becomes available only after its dedicated writer thread signals that it has reached the receive-ready boundary. This startup proof prevents a newly published producer from consuming the complete finite queue before the operating system has scheduled its consumer. It does not enlarge the queue, block forwarding, retry a refused event or claim arbitrary traffic is lossless. Later queue pressure remains counted and reconciled under the same application gap and trailer authorities.
...
Guided calibration workflow checkpoints are also target-owned rows in the local
store, but they are not compatibility facts. Store version 11 adds one positive
store-local workflow identifier, record version, complete immutable target-authority
snapshot, canonical requested, observed, completed, and remaining protocol sets,
bounded attempt ordinal, optional in-flight phase and protocol, closed lifecycle and
pause vocabularies, optimistic revision, and timestamps. Target deletion cascades to
its workflows. Reads validate every closed token and canonical set. Updates name the
expected revision and either advance it exactly once or report missing or concurrent
change. A checkpoint contains no plan or response, capability, credential, key,
certificate authority, trust state, proxy endpoint, target secret, effect obligation,
...
A guided calibration proposal is a deterministic read-only projection over one resolved stored target, one caller-supplied complete-or-unavailable process-image snapshot, the exact native and product version context, optional route and family overrides, concrete observed protocol candidates, and retained compatibility facts. It recognizes only a valid Steam client declaration, one direct client declaration, or an ordered publisher chain with unique roles from launcher to client. Only a complete snapshot with every required image absent may produce a cold runnable case. A warm case requests operator-owned normal application closure, and ambiguous or unavailable state produces no runnable work. Child-environment routing and IPv4 are conservative defaults. Missing, stale, legacy-incomplete, mismatched, negative, and conflicting exact routing evidence each propose only reachability and defer every protocol candidate. Current unconflicted `proxy-routing=reached-client` evidence permits only protocol candidates without current unconflicted `inspectability=full` evidence. The proposal enumerates no processes, starts nothing, changes no trust, opens no listener, writes no artifact or fact, and does not alter ordinary Deep Capture eligibility.

The guided protocol extension accepts a bounded repeatable concrete candidate set as measurement requests, not evidence. A completed session supplies typed terminal observations to one facade policy that admits only identified non-packet-only protocol classifications owned by the final client; controlled-harness ownership is an explicit test-only equivalent. Unknown, unrouted, launcher, intermediate, ambiguous, unavailable, and uncorrelated observations never become automatic candidates. Compatibility protocol and inspectability facts use the same final-client boundary. When one session contains several eligible inspectability states for the same protocol, only its strongest directly observed state is appended, so an expected metadata-only handshake record cannot conflict with a later full application observation from that session. The CLI then re-reads current exact facts and re-runs the proposal. Requested and eligible observed candidates enter the target-bound workflow checkpoint, while current facts remain the only completion authority. Current routing with no candidate remains ordinary Deep Capture ready while protocol coverage is reported as unknown.

The guided target front door preserves stored selection as the first authority. A resolved row continues immediately and a stored ambiguity remains terminal. Only a clean stored miss may run the same bounded Steam and known-roots discovery composition used by explicit discovery. One exact Steam application identifier or Unicode-aware case-insensitive candidate display name produces a complete registration plan; fuzzy, substring, predicted-handle, folder, executable-hint, and path-prefix selection are prohibited. Ambiguous exact candidates are all listed with source, identity, fidelity, classification, name, and install root, and none is selected. The plan binds the complete candidate, conserved discovery account and warnings, the effective local store, the one registration operation, and any deterministic anchored identifier in versioned canonical JSON. Human and machine output expose the full account before confirmation, so incomplete discovery cannot look complete on the successful-candidate path. Interactive confirmation defaults to no. Structured confirmation requires the exact `target-registration-v1` identifier plus one LF through `--authorize-stdin`, independently from any later Deep Capture plan.

After confirmation, discovery is repeated and the complete canonical authority must match before that one candidate reaches `register_candidate`. Registration stays idempotent and uses no source-specific storage path. The inserted or exact already-present row is recovered by canonical anchor or exact install root and passed to the existing proposal by durable identifier. Discovery candidates intentionally store no launch entries, and their executable field remains a findability hint. A new row therefore retains the existing `missing-launch-declaration` limitation rather than acquiring fabricated launch authority. Registration authorization applies to the target row only and never authorizes trust, routing, launch, capture, artifact, or compatibility-fact effects.

A resolved target with any present launch value bypasses guided client setup unchanged. A target whose launch value is absent may enter setup only when it has one positive canonical `steam:<appid>` anchor and current bounded discovery produces exactly one Steam candidate with that application identifier, the exact same non-empty install root, and one verbatim whitespace-free executable value suitable as a Windows image identity. No exact candidate, multiple exact candidates, absent stored install authority, install-root disagreement, absent executable metadata, and unsafe executable syntax remain distinct setup limitations rather than collapsing into generic missing-launch guidance. The appinfo value remains proposal evidence and does not itself identify the socket holder. The versioned `steam-client-setup-v1` plan binds the complete target row, candidate, conserved discovery account and warnings, effective store, proposed executable, resulting one-client declaration, authored fidelity, and explicit absence of process control, process launch, session authorization, proxy, routing, or trust effects. Interactive input asks whether that exact executable holds the target's network sockets and defaults to no. Structured input requires the exact plan identifier plus one LF through `--authorize-stdin`. Registration, client setup, and any later Deep Capture attempt consume separate input lines and no identifier transfers authority between them.

After positive confirmation, the command re-reads the target by stable identifier, repeats discovery, rebuilds the complete plan, and requires byte-equivalent canonical authority. The shared target store then starts an immediate transaction, compares every stored target field to the planned row, and only on an exact match fills `launch_entries` with one client role and stamps fidelity `authored`. It never uses the observed-capture promotion operation, never replaces a resolved, unresolved, malformed, or empty present launch value, and never changes provenance, classification, evidence, install metadata, or compatibility facts. Missing, changed, ambiguous, unavailable, unsafe, negative, unsure, closed, invalid, interrupted, and failed outcomes remain explicit and cause no setup mutation. Every emitted setup plan receives exactly one terminal setup outcome, including target re-read, transaction, store, and post-update verification failures. A successful update is re-read and compared to the exact authored result before the unchanged guided proposal can continue under any independently authorized session plan.
...
The stable library proposal operation supplies the policy for the bounded guided front door:

```text
fragcap calibrate ((<SELECTOR> | --target <SELECTOR> | --id <ID>) [--candidate <CANDIDATE_ID>] [--launch-case <CASE>] [--routing-strategy <STRATEGY>] [--proxy-family <FAMILY>] [--protocol <PROTOCOL>]... | --resume <WORKFLOW_ID>) [OPTIONS]
```

`calibrate` resolves or confirmation-gates one exact target, obtains complete query-only process image snapshots, reads retained compatibility facts, and consumes the proposal with workflow-bound routing and loopback family. Child-environment and IPv4 remain defaults. Repeatable `--protocol` values use the closed concrete calibration vocabulary and are normalized before the proposal; they request measurement and never constitute evidence. Missing or non-positive routing evidence selects reachability first. After every successful case, concrete final-client observations may add bounded candidates, but only a fresh current positive fact permits progression. The command reopens the store, re-resolves the durable target, snapshots process state, and rebuilds S139 before every later attempt. Each selected case delegates independently to the existing low-level executor, including a fresh complete plan, separate confirmation, deadlines, visible session trust when required, lifecycle, artifacts, terminal exit, cleanup, and append-only directly observed facts.

Fresh calibration creates a workflow checkpoint after target registration and topology setup but before the first effectful attempt. It binds the optional launch-case assertion, routing strategy, and address family with the target and protocol intent. `--resume` selects one exact positive identifier only from the effective local store and conflicts with target selectors, candidate selection, exact-case mutation, and new protocol candidates. Resume compares the complete stored target authority with a fresh stable-id resolution, rereads processes and exact facts, and rebuilds S139 from the immutable workflow dimensions. A changed target or mismatched launch assertion refuses the workflow before a new plan. A completed workflow resume is a no-effect current-status check. A row left in-flight by another process first becomes an `interrupted` pause and starts no effect. The next explicit resume reaches the existing recovery inspection before any new S134 plan.

Current exact positive routing with no candidate starts no session, reports protocol coverage unknown, and points to `fragcap deep-capture --id <ID> --local-db <EFFECTIVE_PATH> --launch`. Current positive facts for every candidate report exact coverage completion without claiming universal coverage. A warm target starts no session by default and records a shutdown pause; its continuation uses `fragcap calibrate --resume <WORKFLOW_ID> --local-db <EFFECTIVE_PATH> --restart-warm`. One workflow allocates reachability at most once and each of the thirteen concrete protocols at most once, for a closed maximum of fourteen durable attempt ordinals. Exact case identity includes launch, route, family, phase, protocol, backend, product, and available target version. Selecting a prior case is no progress and starts no session. A decline, invalid or closed input, interruption, terminal failure, missing positive fact, warm transition, target change, proposal limitation, or bundle refusal stops every later attempt and checkpoints exact state. Current facts divide requested and observed sets into completed and remaining work. Paths are PowerShell-quoted so the workflow identifier continues against the same store.

`calibrate --resume <WORKFLOW_ID> --pause-for <login|eula|update|anti-cheat|gameplay|shutdown|interrupted>` atomically records operator-owned work and exits without registration, proposal execution, capture, proxy, trust, launch, cleanup, or process control. Warm state maps conservatively to shutdown, missing required positive evidence to gameplay, session interruption to interrupted, decline to authorization, and terminal failure to failure. A pause is intent, not proof the named action occurred and not compatibility evidence. One additive `calibration.guidance` JSON event carries stable target identity, topology, action, status, launch cases, reason, images, limitations, requested, observed, completed, and remaining protocol arrays, nullable next command, attempt, maximum, phase, protocol, workflow identifier, workflow revision, workflow state, pause reason, and exact resume command fields; human output projects the same facts.

Ambiguous installed-target, Steam client metadata, and eligible stored non-Steam client selection uses one `candidate-v1:` identifier derived from canonical complete candidate authority rather than discovery order. `calibration.choice_required` reports the choice scope, selector, optional target, conserved discovery counts, and reviewable candidates in stable identifier order. No omitted, unknown, malformed, duplicate, stale, or already-consumed identity selects anything. A chosen target still passes the complete registration plan and post-confirmation rediscovery; a chosen Steam executable still passes the separate client plan, exact install-root and app-id checks, post-confirmation rediscovery, and complete-row conditional update. A stored-client choice is available only for a non-Steam target with two or more client-only Windows launch entries. Its separate `stored-client-selection-v1` plan binds the complete current target, selected executable, resulting single authored client declaration, effective store, and no-effect boundary. Fresh target reproduction plus a complete-row conditional update precede proposal evaluation. Steam authority and ordered publisher chains never enter this rewrite path. The candidate digest is an ephemeral narrowing reference, not target identity, workflow state, authorization, topology, or compatibility evidence.

Store version 12 adds one nullable launch-case assertion plus required routing-strategy and address-family values to the version-one workflow record. Existing rows receive their exact prior behavior: inferred launch case, child-environment routing, and IPv4. Fresh `--launch-case`, `--routing-strategy`, and `--proxy-family` inputs become immutable workflow intent; resume conflicts with every intent-changing input. A launch assertion must equal the current proposal's inferred cold case and cannot rewrite topology. The closed routing vocabulary is inspectable, while the proposal continues to refuse every unimplemented strategy before a plan or effect. IPv6 uses the existing S119 listener authority. Human and JSON guidance expose the assertion, effective routing strategy, and family, and every delegated low-level attempt receives those exact workflow dimensions under a fresh plan.

Store version 13 widens only the workflow pause-reason constraint with `update` and `anti-cheat`. The additive migration copies every version 12 checkpoint column and value exactly, changes no workflow record version, and creates no new authorization, evidence, or effect authority.

On a stored selector miss, `calibrate` may discover and register one exact installed candidate. It emits `calibration.registration_plan` with the complete canonical plan, every conserved discovery count, and the warning count, followed by `calibration.registration` with the terminal registration decision, optional resulting stable target identifier, and whether proposal evaluation continued. The human plan renders the complete candidate, discovery account and warnings, and store before input. Structured input is exact and constant-time compared, and fresh discovery must reproduce all plan-bound authority before persistence while retaining exact changed no-match or ambiguity diagnostics. After registration, every downstream resolution replaces the discovery selector with the recovered stable identifier, so a numeric Steam application id cannot be reinterpreted as a listing row. A new discovery row carries no launch authority and can therefore produce the existing missing-declaration limitation without a session. Registration occurs before workflow creation, promotes no discovery hint, and weakens neither ordinary Deep Capture eligibility nor per-attempt plan authorization.

When that exact Steam row has no launch value, `calibrate` may emit `calibration.steam_client_plan` with its `steam-client-setup-v1` identifier, complete canonical plan, durable target id, Steam application id, proposed executable, every conserved discovery count, and warning count. Ineligible authority reports its exact no-candidate, ambiguous-candidate, missing-root, root-mismatch, missing-executable, or unsafe-executable reason. The human projection asks only whether the displayed executable holds the sockets. `calibration.steam_client` records the terminal decision and whether proposal evaluation continued, including a failed terminal record before any target re-read, persistence, or post-update verification error returns. Positive confirmation is followed by fresh target and discovery reproduction plus a complete-row conditional store update; declined or stale authority leaves the row untouched. On success, the durable target is re-resolved with one authored client and proceeds into existing guidance. This assertion does not claim that fragcap observed ownership and does not authorize the later session.
...
Observation duration governs ordinary Capture execution. Capture stop, proxy stop and terminal proxy observation drain consume the one displayed shutdown ceiling from a shared shutdown origin. The drain returns explicit complete or incomplete structured authority and retains any partial evidence. Complete cached terminal evidence remains complete even when collection occurs after the earlier Capture duration; incomplete, failed or over-budget terminal work remains a distinct failure. No hidden grace period or restarted shutdown budget is permitted.
...
### 22.7 Native product documentation and independent-review readiness

Current architecture, setup, Capture modes, CLI, compatibility, refusals, outputs and correlation, Doctor and recovery, security and privacy, finite bounds, packaging and migration, stable API and completion-status guidance trace to native source and controlled evidence. Current fenced fragcap commands across README and nonhistorical site pages are parser-checked without dispatch; Mermaid is not a command fence, and retired Deep Capture consent inputs are refused. Artifact specimens link to published schema/examples validated by actual product readers, exact generated goldens or controlled product contracts, not approximate new JSON or the target-schema validator.

The thirteen-topic `docs/audits/native-documentation-coverage.v2.json` contract is checked by `cargo xtask docs check` against current authored pages, exact required section headings, five closed example authorities and Cargo-discoverable non-ignored tests under explicit features. Missing, duplicate, unknown, reordered, historical-as-current, unsafe, absent, ignored or feature-inapplicable references fail. Published v0.10.2 identity is read from `docs/published-release.json`; the retained S154 through S157 lineage names the engineering slices that established the contract. The contract proves documentation engineering traceability, not every prose statement or independent whole-product acceptance.

The stable reader-facing Native product contract route maps all thirteen topics and explains command-parser, manifest-reader, packet-golden, controlled-bundle and stable-API authority. It preserves independent review #333, Doctor field validation #372, documentation reconciliation #331, final gate #334 and epic #278 as separate open outcomes. S158 closed the Windows QUIC observation-queue correction formerly tracked by #413. Later actual findings amend the bounded current contract before the parent can close; automation never invents them.
...
`cargo xtask guided-calibration-acceptance` validates the versioned issue #380 registry in the ordinary Linux and Windows sequence. It requires exactly thirteen criteria, controlled automated evidence, tracked Rust source paths, and exact non-ignored test functions. Portable evidence must be unconditional; Windows evidence must be explicitly Windows-gated and is executed by the ordinary Windows workspace test leg. The gate rejects stale, duplicate, malformed, missing, disabled, or documentary references. It neither runs nor accepts a real game, real account, remote game service, capture driver, real trust mutation, or sensitive live capture as implementation evidence.
...
Ordinary uninstall preserves every user-owned database, profile, capture, log, cache, and Deep Capture bundle. Interactive removal separately offers an unchecked current-user fresh-start choice. Its confirmation names the initiating user's exact `%APPDATA%\fragcap` and `%LOCALAPPDATA%\fragcap` roots and warns that deletion is irreversible. The MSI passes those exact client-context roots to the installed executable before removing it; it never resolves an elevated service account's profile and never implements its own recursive path search. The deferred action verifies the supplied roots against the impersonated initiating user, previews the current inventory, and passes that identifier into execution, so no process-visible installer flag bypasses confirmation. Silent removal requires both `FRAGCAP_FRESH_START=1` and `FRAGCAP_FRESH_START_SCOPE=current-user`. Repair, reinstall, upgrade, rollback, and automatic maintenance ignore this authority.

An administrator who needs all-users cleanup must use `fragcap fresh-start --scope all-users --preview`, review every exact SID-bound root, and then execute with that preview identifier and `--yes`. Another profile's Deep Capture sessions remain until current-user cleanup in that profile reconciles its Windows CurrentUser certificate authority; the elevated administrator must not replay Doctor under the wrong identity. The static MSI does not offer an all-users checkbox because it cannot display and bind a dynamic profile inventory truthfully. Custom `FRAGCAP_*` roots, command-line paths, exported evidence, Npcap, Wireshark configuration, and independently managed extcap registrations remain excluded. Link or reparse traversal is refused rather than followed.
...
Published-package replay is a disposable hosted Windows evidence tier over immutable user-downloadable bytes rather than a locally rebuilt candidate. The workflow verifies all six v0.10.2 file sizes and digests before effects, then reuses package certification to run distinct portable and installed controlled native smoke under exact-program firewall containment. Report schema 4 preserves executable identity, exact ordered structured native proxy-start and same-session terminal reached-client evidence, distinct session identity digests across surfaces, zero unexpected process or non-loopback observations, bounded socket diagnostics and cleanup reconciliation for each surface. Rust validates the bounded event sequence, owns the predicate codes and projects its closed session summary before the PowerShell orchestrator builds the report. Socket polling remains negative evidence when it observes a violation but is not positive authority, because absence from a bounded polling snapshot does not establish absence from the execution. This is reproducible implementation-authored evidence, not independent whole-product acceptance. The S158 Windows observation correction is included in the immutable candidate; independent review #333 remains external.
...
**Tier 3, live.** Optional operator-owned manual compatibility verification against an already published release. It may verify end-to-end acquisition through the launcher chain, attribution accuracy against SC-7, and analyzer integration for those exact published bytes. An agent does not launch the sensitive application or automate this tier. Absence of Tier 3 evidence does not block controlled implementation acceptance, and S147 records no live compatibility claim. Any retained result belongs in a private compatibility record; only a scrubbed summary may enter public project records.
...
S158 addresses the later post-S157 recurrence without superseding S150's batching result. The application lease is unpublished until its writer reports receive readiness, and a deterministic held-start regression proves producers cannot enter the queue before that boundary. The 4,096-event queue, 64-event pending-storage bound, 64 KiB buffer, fourteen cases, seven windows, timing budgets, loss invariants and conservation equation remain unchanged. Fresh Windows performance acceptance requires a first-attempt pass on the final pull-request head; an identical-head retry remains diagnostic only.

S160 supersedes S158's capacity conclusion after merged-main Windows evidence showed that an already-ready writer can be descheduled while the finite QUIC burst is produced. Product sessions and the canonical performance harness now share a 16,384-event application queue while forwarding remains independent and admission remains nonblocking. A deterministic post-readiness stall fills the exact bound without loss, proves the next event is refused and counted, then proves ordered drain and zero terminal ownership. Performance report schema 2 requires exact application event attempts and queue capacity, refuses a canonical burst that exceeds the capacity even if concurrent draining kept the observed peak low, and partitions writer-observed payload into retained, intentionally omitted, and storage-dropped bytes before adding pre-writer queue loss to the total. Queue-loss authorities use original observed lengths even when retained streaming payload is omitted or truncated. Historical loss-free schema 1 evidence remains readable, unknown versions remain refused, the registry remains schema 1, and first-attempt Windows acceptance remains mandatory.
...
During an operator-owned tier 3 session against an already published release, fragcap records a high-cadence socket table
...
measurement tool is `cargo xtask accuracy`, and its output is retained in the private compatibility record. Only a scrubbed summary may enter the public repository.
...
Fresh-start cleanup calls this same recovery implementation before removing any canonical session tree or owner registry. Its separate data-deletion confirmation does not broaden ambiguous legacy recovery authority. Active, malformed, wrong-store, mismatched, unsupported, concurrently owned, or failed obligations retain their journals and sensitive evidence, force a partial result, and direct the operator to `fragcap doctor --fix`. Independently owned ordinary data may still be removed, but every retained path remains in the bounded local report and a partial operation is never described as a complete wipe.
...
S139 is a bounded child of the guided calibration workflow in #380. It adds one stable pure proposal authority that derives strict stored launch topology, requires caller-proven cold state, applies child-environment and IPv4 defaults, and selects only useful exact S121 reachability or protocol measurements with typed limitations and evidence reasons. This deliberate split establishes reusable policy before any guided CLI, resolution, process enumeration, execution, persistence, or ordinary-eligibility change. Issue #392 closes with S139 while #380 remains open.

S140 is the next bounded child of #380. It adds `fragcap calibrate` for existing targets, one complete query-only process image inventory, stable guidance, and at most one exact reachability delegation through the unchanged plan-bound executor. Current evidence, warm state, and typed limitations remain effect-free unless the operator explicitly requests the existing warm close-and-retry workflow. Issue #394 closes with S140 while TLS and protocol guidance, automatic registration, multi-attempt execution, and workflow persistence remain in #380.

S141 is the next bounded child of #380. It adds repeatable concrete guided protocol requests after current exact reachability, executes only the first S139 protocol step under a separately displayed authorization plan, derives continuation candidates only from eligible final-client terminal observations, and re-reads exact facts before reporting completed or remaining coverage. Requested protocols remain distinct from observations and facts. Issue #396 closes with S141 while automatic registration, internal multi-attempt execution, workflow persistence, resume, and final completion remain in #380.

S142 is the next bounded child of #380. It preserves stored target selection as authoritative and reaches the existing bounded discovery composition only after a clean miss. One exact Steam application identifier or Unicode-aware case-insensitive display name can produce a complete registration plan, which binds the candidate, conserved discovery account and warnings, and effective store before default-no or exact structured confirmation. Fresh discovery must reproduce the same complete authority before the shared registration operation. The resulting durable target enters S139-S141 without promoting executable hints into launch authority, so unresolved topology remains an explicit limitation rather than a fabricated session. Issue #398 closes with S142 while internal multi-attempt execution, workflow persistence, resume, and final completion remain in #380.

S143 is the next bounded child of #380. It offers one confirmation-gated Steam client setup only when a durable target's launch field is truly absent and fresh exact application and install-root authority supplies one suitable appinfo executable proposal. The complete plan binds every source and resulting field while explicitly authorizing no session effect. Only the operator's positive socket-holder assertion can reach an immediate complete-row conditional target update, followed by exact re-resolution. Present declarations, drift, races, and every non-positive response remain unchanged. Issue #400 closes with S143 while non-Steam topology authoring, internal multi-attempt execution, workflow persistence, resume, and final completion remain in #380.

S144 is the next bounded child of #380 and deliberately supersedes S141's process-exit boundary while preserving its safety reason. One invocation may execute the current S139 reachability step and each later useful requested or eligible observed protocol step, but every attempt receives fresh target, process, fact, proposal, bundle, S134 plan, and confirmation authority. Only a fresh current positive fact permits the next distinct exact case. Decline, invalid input, interruption, failure, partial evidence, warm state, limitations, repetition, or the closed fourteen-attempt bound stops later effects and reports exact coverage plus continuation. Issue #402 closes with S144 while non-Steam topology authoring, persistent workflow state, resume, richer operator exercise pauses, and final completion remain in #380.

S145 is the next bounded child of #380 and deliberately supersedes S144's stateless cross-process continuation. Store version 11 adds one target-bound revisioned workflow checkpoint before the first effect, including a bounded canonical history of exact attempted cases so S144's no-repeat rule survives process exit. Explicit `--resume` revalidates the complete target snapshot and rebuilds current process, fact, proposal, recovery, bundle, plan, and confirmation authority. Durable in-flight state records interruption, never completion or reusable authority. Explicit login, EULA, gameplay, shutdown, and interruption pauses are no-effect operator intent. Issue #404 closes with S145 while non-Steam topology authoring, ambiguous candidate resolution, advanced override policy, and final parent completion remain in #380.
...
S137 closes #377 at the explicit fresh-start boundary. Preserve-by-default MSI behavior remains unchanged unless both destructive properties are present during explicit removal. Current-user cleanup receives the initiating user's exact canonical roots, inventories without following redirections, reuses Doctor's exact Deep Capture recovery authority, deletes only accepted entries, and reports complete or partial truth. All-users scope is a distinct elevated preview-and-confirm command whose identifier binds the complete profile inventory. Package certification uses synthetic canonical roots to prove preservation, exclusions, recovery, cleanup, and clean reinstall without risking runner profile data. S137 adds no dependency or Deep Capture completion claim.
