...
S143 (`specs/143-guided-steam-topology`) follows S142 and closes child issue #400 at the confirmation-gated Steam client-authoring boundary. Only a durable Steam target whose launch field is absent can join one fresh exact application and install-root discovery candidate. Its appinfo executable remains a displayed proposal until the operator explicitly asserts that it holds the target's sockets under a separate domain plan. Target and discovery authority are reproduced after input, then one immediate complete-row conditional store operation fills only the client declaration and authored fidelity before exact re-resolution. Every present launch value, drift, race, and non-positive response remains unchanged. S143 adds no dependency, package, schema, non-Steam topology authoring, internal multi-attempt loop, resume, or completion claim, and #380 remains open.
