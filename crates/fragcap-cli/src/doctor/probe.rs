...
/// both are gone.
fn live_probe(npcap_present: bool) -> (Vec<super::IfaceInfo>, Option<bool>, Option<String>) {
    #[cfg(all(feature = "live", windows))]
    {
        ...
        // fragcap::detect_driver call pcap::Device::list, and when npcap is absent
        // that load raises a delay-load exception (0xC06D007E, MOD_NOT_FOUND) that
        // aborts the process before the Result-based guards inside those functions
        // can run. So the live backend is touched only when npcap is present. When
        // it is absent nothing was attempted, so this is not a probe failure: the
        // interface set is empty with no error, and the npcap check (not the
        // interface check) carries the remediation. Without this gate `doctor`
        // could not run on a machine with no npcap, which is the one machine that
        // most needs it to run and say "install npcap".
        if !npcap_present {
...
pub fn gather() -> Inputs {
    ...
    #[cfg(not(windows))]
    {
        ...
        // No npcap on a non-Windows build; the live backend is not linked anyway.
...
fn gather_windows(user_count: usize) -> Inputs {
    ...
    // npcap presence gates every touch of the live backend below. enumerate and
    // detect_driver reach the delay-loaded wpcap.dll, which aborts the process
    // with a delay-load exception when npcap is absent (see live_probe), so the
    // backend must not be called first. npcap is present when its own wpcap.dll
    // exists in the Npcap directory; the WinPcap API compatibility option
    // additionally installs wpcap.dll directly into System32, so its presence
    // there is the signal for that option.
    let npcap_present = npcap_wpcap.exists();

    // The live backend answers what exists: the interface set, and whether a
    // loopback adapter is among them. Probed only when npcap is present, and
    // linked only under the `live` feature, so this returns empty and
    // undetermined otherwise.
    let (interfaces, loopback_supported, interface_error) = live_probe(npcap_present);
