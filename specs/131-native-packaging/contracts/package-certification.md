...
The exact extracted and installed executables must report the certified build identity, start with Npcap absent, report the native Deep Capture backend, and complete the existing controlled loopback native smoke under a sanitized environment. The harness blocks non-loopback traffic for the exact executable, samples its complete descendant tree and sockets while it runs, and rejects every unexpected executable or observed non-loopback endpoint. Successful controlled route completion under that enforced boundary supplies the positive loopback result without requiring a polling snapshot to catch a transient socket. No undeclared child process, non-loopback connection, network fetch, package manager, Python executable, external proxy executable, first-use installer, trust residue, route residue, or target mutation is allowed.
