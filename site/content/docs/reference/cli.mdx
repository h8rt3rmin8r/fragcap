...
Authorization plan and prompt writes must succeed before input is read. Interactive approval requires a complete LF-terminated line. The prepared CA must remain within its displayed validity period, and the selected target authority must match both the immediate post-approval check and the facade's final resolver. Before constructing a new plan, a bounded read-only inspection refuses any pending prior-session recovery and directs the operator to `fragcap doctor --fix`; the new plan never authorizes effects against an older session.
