...
The exact extracted and installed executables must report the certified build identity, start with Npcap absent, report the native Deep Capture backend, and complete the existing controlled loopback native smoke under a sanitized environment. The harness blocks non-loopback traffic for the exact executable, samples its complete descendant tree and sockets while it runs, rejects every unexpected executable, and records product-process, permitted-system-process, and endpoint counts including blocked non-loopback attempts. The only permitted system descendant is the exact `conhost.exe` beneath the current Windows system directory because Windows may attach its console host to a console-subsystem child even when the application requests hidden execution. Schema 4 requires one exact structured native proxy-start event followed by one exact terminal reached-client calibration event from the same session as positive authority, plus distinct session identity digests across the portable and installed rows. Rust parses and validates that bounded ordered stream before the PowerShell harness can project its closed session summary. The exact-program firewall rule proves containment. Socket sampling remains independent negative and diagnostic evidence: any sampled non-loopback endpoint fails, while zero sampled endpoints and no exact loopback sample are valid because polling absence proves nothing. No undeclared child process, successful non-loopback connection, network fetch, package manager, Python executable, external proxy executable, first-use installer, trust residue, route residue, or target mutation is allowed.
