...
Deep Capture is functional but incomplete. S105 adds native HTTP/2,
protocol-faithful metadata, bounded body observations, and a live versioned
application stream to the HTTP/1.1, CONNECT, and HTTPS path. Trust effects use
native Windows APIs. [Issue #278](https://github.com/h8rt3rmin8r/fragcap/issues/278)
...
S103 gives that crate authenticated loopback admission, bounded upstream policy,
per-session certificate and exact trust ownership, raw event accounting, and a
controlled lab for every required protocol family. S104 adds bounded HTTP/1.1,
CONNECT, and separately verified HTTPS. S105 adds multiplexed HTTP/2, complete
available HTTP metadata, bounded streaming body evidence, and the live version
2 application stream. S106 adds bounded WebSocket frame and message evidence,
incremental Server-Sent Events, schema-free gRPC envelopes, proxy-owned TLS key
logs, and explicit upstream client certificates. HAR, correlation, launch and transport coverage, and final
...
Deep Capture starts from one stored target. Its read-only authorization preparation resolves the launch case, current compatibility evidence, proxy backend, bundle path, normalized routing policy, and exact process-local CA identity before creating the session directory, reserving the listener, or starting Capture and the proxy.
...
| Authorization | Exact versioned plan identifier | One target launch authority, process-local CA, routing policy, artifact set, deadline set, and cleanup contract | Decline, mismatch, malformed input, EOF, or drift leaves session effects absent |
...
Interactive execution displays and confirms one complete plan. Structured execution uses `--authorize-stdin` and returns only the exact current plan identifier in the same process. Trust is never silently added to the local-machine store or broadened to an unrelated CA. The former Deep Capture `--trust-ca` and `--yes` booleans are rejected migration inputs.
