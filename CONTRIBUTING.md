...
Npcap remains a separately installed prerequisite for live packet capture. fragcap never bundles, hosts, caches as its own, or redistributes Npcap or its installer. After explicit interactive confirmation, `fragcap doctor --fix` may fetch the vendor's own signed installer from the official location and launch it. The default workspace build and offline tests require neither Npcap nor administrative privilege.
