...
description: "The JSON schema a profile declares: game identity, capture defaults, and the stages that match processes."
---

A profile is a JSON file that names a game and describes the processes fragcap
...
| `kind` | yes | The artifact form. For a profile, `"profile"`. |
| `fidelity` | yes | The trust tier an author declares: `authored`, `verified`, or `heuristic-unverified`. (`observed` is a fourth tier the schema allows for runtime results, but it is stamped by the observation provider, not claimed by an author, and a profile that declares it is rejected.) |
| `game` | yes | The game's identity (below). |
| `capture` | no | Capture defaults (below). |
| `stage` | yes | One or more `stage` entries; at least one. |
| `notes` | no | Free-form human context, carried as data rather than a comment. |
...
Each entry in the `stage` array names one process fragcap watches for and how its
lifecycle affects the capture. A profile has one or more stages.
...
An object of predicates. All that are present must hold for a process to
