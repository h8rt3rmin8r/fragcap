...
Deep Capture starts from one stored target. Its read-only preflight resolves the launch case, current compatibility evidence, Capture readiness, proxy backend, bundle path, and trust authorization before creating the session directory or starting the proxy.

```mermaid
flowchart LR
  target["Stored target"] --> preflight["Read-only preflight"]
...
The current real-target path is deliberately narrow. The selected target must have current `proxy-routing = reached-client` and `proxy-propagation = confirmed` facts for `steam-protocol-cold`. Unknown, stale, conflicting, or wrong-launch evidence is refused. An already-running Steam process is a warm launch fragcap does not own, and direct-executable managed launch is not implemented in v0.7.0, so both are refused before session side effects. The [Deep Capture compatibility reference](/docs/reference/deep-capture-compatibility) carries the complete current traffic and launch matrix.

After preflight, fragcap starts a loopback `mitmdump` child, manages any authorized CA trust, prepares ordinary Capture, and launches the target through Steam with launch-scoped proxy environment. The proxy and packet paths run beside each other. Traffic can appear in packet truth without producing an application observation.

## Trust and routing boundaries

| Boundary | Trigger | Scope and owner | Cleanup or refusal |
| --- | --- | --- | --- |
| Compatibility | Current exact routing facts for `steam-protocol-cold` | One selected stored target and launch case, evaluated by fragcap | Refusal occurs before the proxy, trust, launch, or bundle side effects |
