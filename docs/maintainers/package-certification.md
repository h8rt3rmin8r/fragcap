...
The MSI owns its fixed Program Files payload, related-product registration, exact system PATH entry, and only a Defender exclusion whose exact normalized path is recorded by its private ownership marker after successful creation. A pre-existing administrator-owned exclusion has no marker and survives uninstall. Per-user databases, captures, bundles, and independently managed extcap registration are never installer-owned.
