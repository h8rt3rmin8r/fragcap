...
pub fn run(root: &Path) -> io::Result<usize> {
    ...
    if problems.is_empty() {
        ...
        let tests = registry["threats"]
            .as_array()
            .into_iter()
            .flatten()
            .map(|threat| threat["tests"].as_array().map_or(0, Vec::len))
            .sum::<usize>();
        println!(
...
fn validate_test_references(
    ...
    let mut problems = Vec::new();
    for threat in registry["threats"].as_array().into_iter().flatten() {
        let id = threat["id"].as_str().unwrap_or("<missing>");
        for test in threat["tests"].as_array().into_iter().flatten() {
            let path = test["path"].as_str().unwrap_or_default();
            let function = test["function"].as_str().unwrap_or_default();
            if path.is_empty() || function.is_empty() {
                continue;
            ...
            if !confined || !path.ends_with(".rs") {
                problems.push(format!(
                    "threat {id} test path is not a repository Rust source: {path}"
                ));
                continue;
            }
            if !tracked.contains(&path.replace('\\', "/")) {
                problems.push(format!("threat {id} test path is not tracked: {path}"));
                continue;
            }
            ...
            let source = match fs::read_to_string(&candidate) {
                ...
                Err(error) if error.kind() == io::ErrorKind::NotFound => {
                    problems.push(format!("threat {id} references missing test path {path}"));
                    continue;
                }
                Err(error) => return Err(error),
            };
            if let Err(reason) = validate_test_source(&source, function) {
                problems.push(format!("threat {id} {reason}: {path}::{function}"));
            }
        }
    }
    Ok(problems)
...
fn validate_test_source(source: &str, function: &str) -> Result<(), &'static str> {
    ...
    let Some(position) = source.find(&marker) else {
        return Err("references missing test");
    };
    let prefix_start = source[..position].rfind("\nfn ").map_or(0, |p| p + 1);
    let attributes = &source[prefix_start.saturating_sub(256)..position];
    if !(attributes.contains("#[test]") || attributes.contains("#[tokio::test")) {
        return Err("reference is not an attributed test");
    }
    if attributes.contains("#[ignore") {
        return Err("references ignored test");
    }
    Ok(())
}
