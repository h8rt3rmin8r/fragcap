...
- 2026-09-05: Make the release helper pass explicit native argument arrays, update portable release-bound conformance metadata and staged-binary assertions, and preserve the exact product identity of retained physical Windows evidence. Ordinary static validation accepts that historical identity, while the explicit current-release gate remains strict. This keeps repeatable release assembly separate from physical-host evidence collection and prevents release preparation from rewriting proof it did not rerun.
