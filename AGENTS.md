...
rechecked by the native destination policy on every attempt. Plan and bundle
...
rows plus short and manual two-hour soak profiles before measurement. Every
...
product dependency or completion claim. The final reviewed soak was stopped by
explicit project-owner direction after 1,916 zero-failure case terminals across
4,316 seconds; its raw prefix remains incomplete while the sanitized acceptance
...
is next, and Deep Capture remains incomplete until #334. The runner takes the
...
S131 closes #329 at the final Windows package-certification boundary. A versioned contract binds the three primary downloads and checksum sidecars to the exact six-file ZIP/MSI payload, reviewed size ceilings, x86_64 PE machine and imports, machine-readable native build identity, current unsigned state, exact installer ownership, six finite lifecycle cases, bounded public evidence, and publication order. Pull-request and tag automation build candidate bytes once, certify those final bytes and real installer transitions on fresh Windows, and independently revalidate the transferred bundle before GitHub release creation. Crate publication remains last and separately approved. S131 adds no product runtime dependency; the stable Rust API remains #330 and Deep Capture remains incomplete until #334.

S132 closes #330 at the stable public Rust API boundary. The versioned `fragcap::deep_capture::api` module exposes product-owned configuration, preparation, authorization, lifecycle, routing, native backend, observation, classification, calibration, artifact, recovery, cancellation, and terminal-report contracts without promoting protocol engines or writer implementations. Checked builders provide additive construction, public failures implement the standard Rust error trait, coordinators and injected adapters remain serial and thread-confined, and the cancellation token alone is explicitly `Send` and `Sync`. A compile contract checks the curated inventory and CLI policy path, while a runnable no-CLI controlled example exercises the production native backend with exact cleanup. S132 adds no dependency or package change; final documentation remains #331 and Deep Capture remains incomplete until #334.

S133 closes #375 at the target-discovery integrity boundary and deliberately supersedes the prior plan to assign S133 to #331. Broad discovery and automatic persistence are now separate conserved decisions: Steam application identities remain authoritative, while path candidates require verified engine evidence for hero or Doctor registration. Exact Steam client subtrees are pruned before generic known-root classification. Explicit discovery remains non-persistent and gains a privacy-safe aggregate mode. Historical legacy rows can be previewed and confirmed for atomic removal only when exact tool provenance plus an exact client root, affirmative infrastructure root, stored anchored duplicate, or multi-engine aggregate establish ownership; authored, anchored, incomplete, changed, unrecognized descendants, and ambiguous rows remain. Issue #331 stays open for later documentation work. S133 adds no dependency, schema, or package change.
...
S141 closes #396 at the one-attempt guided protocol boundary. Repeatable concrete `fragcap calibrate --protocol` values remain measurement requests, current exact reachability gates one first S139 protocol step, and each invocation retains a separately authorized session. Typed terminal observations yield automatic continuation candidates only for the final client, then a fresh exact fact read reports requested, observed, completed, and remaining coverage. Unresolved candidates travel in the next durable command rather than storage. Protocol facts now require final-client ownership and retain the strongest directly observed same-session inspectability value per protocol. S141 adds no dependency, package, artifact or storage schema, automatic registration, internal multi-attempt loop, resume, or completion claim, and #380 remains open.

S142 closes #398 at the confirmation-gated guided registration boundary. Stored target resolution remains authoritative. Only a clean miss reaches the existing S133 discovery composition, and only one exact Steam application identifier or Unicode-aware case-insensitive display name can produce a plan. The plan binds the complete candidate, conserved discovery account and warnings, and effective store, is flushed before default-no or exact structured input, and is recomputed from fresh discovery before the one shared registration operation runs. Changed selection retains its exact ambiguity diagnostic. Registration consent is distinct from Deep Capture authorization. The resulting durable row replaces the discovery selector with its stable identifier throughout S139-S141 without converting executable hints into launch authority, so new discovery rows retain the existing missing-declaration limitation. S142 adds no dependency, package, schema, bulk auto-registration, internal multi-attempt loop, resume, or completion claim, and #380 remains open.

S143 closes #400 at the confirmation-gated Steam client-authoring boundary. Only a durable target with an absent launch field, one positive canonical Steam anchor, and one fresh exact application and install-root discovery candidate can produce the separate plan. The whitespace-free appinfo executable remains proposal evidence until the operator positively asserts that it holds the sockets. Ineligible authority is reported by exact condition. The plan binds the complete target, candidate, conserved discovery, effective store, proposed and resulting values, and a no-effect list. After input, fresh authority must reproduce byte-for-byte before an immediate complete-row conditional transaction fills only the client declaration and authored fidelity; every emitted plan receives a terminal outcome across target re-read, store, and post-update verification failures. Every present value, drift, race, and non-positive response remains unchanged. Registration, setup, and session authorization stay separate. S143 adds no dependency, package, schema, non-Steam topology authoring, internal multi-attempt loop, resume, or completion claim, and #380 remains open.

S144 closes #402 at the bounded guided calibration sequence boundary and deliberately supersedes S141's one-process-per-attempt choice while preserving separate authorization. Every useful reachability or protocol attempt freshly re-resolves the durable target, snapshots process state, reads exact facts, rebuilds S139, derives a distinct bundle, emits a complete S134 plan, and consumes its own response. Only a fresh current positive fact advances to a different exact case. One reachability plus thirteen concrete protocols is the closed bound. Decline, invalid input, interruption, failure, partial evidence, warm state, limitations, target drift, repetition, and bundle refusal stop later effects with exact coverage and continuation. Requested candidates, eligible final-client observations, and current facts remain distinct. S144 adds no dependency, package, storage or artifact schema, non-Steam topology authoring, persistent workflow state, resume, or completion claim, and #380 remains open.

S145 closes #404 at the durable guided calibration pause-and-resume boundary and deliberately supersedes S144's stateless cross-process continuation. Local-store schema version 11 adds one target-bound versioned checkpoint with canonical candidate and coverage sets, a bounded exact-case no-repeat history and attempt ordinal, closed lifecycle and pause vocabularies, and optimistic revision. `calibrate --resume <WORKFLOW_ID>` revalidates the complete target snapshot and rebuilds current process, fact, proposal, recovery, bundle, plan, and response authority. A row left in flight records interruption and starts no effect on its first resume. Login, EULA, gameplay, shutdown, and interruption are explicit no-effect operator pauses; authorization and failure remain exact terminal pauses. No checkpoint contains a plan, response, secret, trust state, endpoint, effect obligation, or compatibility claim. S145 adds no dependency, package, network behavior, artifact schema, non-Steam topology authoring, ambiguity resolution, advanced override, or completion claim, and #380 remains open.

S146 closes #406 at the explicit guided calibration choice and exact-case intent boundary. Ambiguous discovered targets and Steam executable metadata now carry canonical content-derived `candidate-v1` identities rather than positional selection. One current exact identity must narrow the existing confirmation plan, and duplicate, stale, malformed, unknown, already-consumed, or unused values stop before writes or effects. Local-store schema version 12 adds immutable optional launch-case assertion, routing strategy, and address family to each workflow, with prior behavior migrated exactly. Resume cannot mutate those dimensions, a launch assertion cannot contradict current inferred topology, unsupported routing remains a proposal refusal, and IPv6 reuses S119 authority. S146 adds no dependency, package, process control, new routing implementation, trust path, artifact schema, non-Steam topology authoring, or completion claim, and #380 remains open.

S147 closes guided-calibration parent issue #380 at the controlled implementation-acceptance boundary. A versioned registry maps all thirteen criteria to exact non-ignored Rust tests and the ordinary Linux and Windows CI gates validate its complete inventory and source currency. The audit adds a separate confirmation-gated plan for choosing one exact ambiguous non-Steam client-only launch entry, without rewriting Steam or publisher chains; store version 13 adds update and anti-cheat to the no-effect pause vocabulary while preserving version 12 checkpoints exactly. Controlled tests prove inferred direct, Steam, and publisher defaults, complete visible plan authority, durable resume, refusal, cleanup, coverage, and ordinary Deep Capture handoff. S147 adds no dependency, lockfile package, network behavior, trust path, or artifact schema. It does not close general Deep Capture issue #334, and it did not demonstrate live game compatibility. Optional real-game validation belongs only to the operator against an already published release.

S148 closes #379 at the embedded Deep Capture workflow-discovery boundary. Root, Doctor, target, calibration, Deep Capture, and bundle long help form one first-session journey with complete parser-checked Steam and direct or publisher examples. The Deep Capture page orders ordinary controls before advanced and sensitive controls, states exact-case compatibility and certificate-pinning limits, and labels sensitive artifacts. Human pre-session refusals and cleanup failures provide one bounded next command while JSON and runtime terminal contracts remain unchanged. S148 adds no dependency, lockfile package, schema, network, trust, target-launch, or sensitive-data behavior. General Deep Capture issue #334 remains open.

**Current completion policy.** The verified v0.10.2 release contains the native
Deep Capture implementation and current documentation boundary completed through
S159. Constitution 1.5.0 retires standing verification issues: implementation
issues close when their scoped behavior, documentation, and repository-controlled
gates are complete. Later failures observed against an active release become
specific defects. This completion state does not claim universal target
...
- Do not close an issue merely because it was included in a slice. Close it after its scoped acceptance criteria and repository-controlled gates are complete.
- Do not keep an implementation issue open, or create a standing issue, solely to await future operator testing, field measurement, independent review, release observation, or unspecified confirmation. File a concrete defect against the affected active release when an actual failure is observed.
...
- Keep planned capabilities visibly distinct from shipped capabilities. Closing an issue or milestone must not cause documentation, associations, release notes, or public claims to advertise behavior that has not passed the repository's implementation and release gates.
- When completed historical work must be represented, create a clearly identified retrospective issue or milestone, attach the available implementation and release evidence, and close it as completed without rewriting project history.
