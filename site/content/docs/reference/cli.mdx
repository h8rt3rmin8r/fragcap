...
This page describes the current source command tree, checked by the production parser without dispatch. The published v0.10.1 baseline includes the native API, guided calibration, command migrations below and S151's visible Doctor `--timings`, nested readiness timings and live elapsed diagnostics. Historical v0.10.0 retains the hidden completion-only timing option. Hidden test controls are excluded. clap's generated `help` and `version` controls remain available but are not repeated in every table.
...
Older releases and their preserved changelog pages may refer to command forms that the current CLI no longer accepts. Use these replacements with the published v0.10.1 command surface:
...
| `--ring` | open | none | all | Set the bounded ring window as a duration or size. See [Capture modes](/docs/guides/capture-modes) for mode and sink constraints. |
...
`--roles` is enforced. `--direction in` and `--direction out` are recorded and reported, but v0.10.1 still writes both directions.
...
| `--timings` | open | `false` | all | Published in v0.10.1: show phase and nested readiness completion timings on interactive stderr progress. No JSON or final report changes. Historical v0.10.0 accepts this option but hides it and supplies only coarse completion timings. |
