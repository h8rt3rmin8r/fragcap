...
`targets show` is read-only. It does not start a proxy or target, change trust, contact a catalog, or refresh evidence. Ordinary Deep Capture requires current facts proving that scoped proxy routing reached the final client through a cold Steam protocol launch:
...
Proceed only when the routing fact is current for `steam-protocol-cold`. Propagation, protocol behavior, CA acceptance, and inspectability describe separate observations without replacing that routing requirement.

<Callout type="warn" title="Unknown requires an explicit measurement">
If the detail view reports `unknown`, stale or conflicting routing facts, or evidence for another launch case, do not run ordinary Deep Capture. Close Steam and run reachability calibration for the exact target and `steam-protocol-cold` launch case. Do not infer compatibility from the title, engine, launcher, executable, or another target. See the [Deep Capture compatibility reference](/docs/reference/deep-capture-compatibility).
</Callout>
