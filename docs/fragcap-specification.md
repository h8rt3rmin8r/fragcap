...
Current exact positive routing evidence starts no session and points to `fragcap deep-capture --id <ID> --launch`. A warm target starts no session by default and points to `fragcap calibrate --id <ID> --restart-warm`; that explicit option reuses the existing operator-owned close-and-retry authority before any calibration plan. Ambiguous, invalid, or unavailable proposals report every typed limitation before effects and invent no command. One `calibration.guidance` JSON event carries stable target identity, topology, action, status, launch cases, reason, images, limitations, and nullable next command; human output projects the same facts. S140 does not register targets, select TLS or protocol cases, run multiple attempts, persist workflow state, or weaken ordinary Deep Capture eligibility.
