...
S142 closes #398 at the confirmation-gated guided registration boundary. Stored target resolution remains authoritative. Only a clean miss reaches the existing S133 discovery composition, and only one exact Steam application identifier or case-insensitive display name can produce a plan. The plan binds the complete candidate and effective store, is flushed before default-no or exact structured input, and is recomputed from fresh discovery before the one shared registration operation runs. Registration consent is distinct from Deep Capture authorization. The resulting durable row enters S139-S141 without converting executable hints into launch authority, so new discovery rows retain the existing missing-declaration limitation. S142 adds no dependency, package, schema, bulk auto-registration, internal multi-attempt loop, resume, or completion claim, and #380 remains open.
