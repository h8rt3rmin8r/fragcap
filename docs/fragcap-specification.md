...
| 0.1.63-draft | 2026-09-10 | W. Thompson | **Adds confirmation-gated guided target registration (issue #398).** Extends sections 15, 17.2.1, 25, and 28. Stored target resolution remains first. Only a clean miss reaches the existing bounded discovery composition, where one exact Steam application identifier or case-insensitive display name can produce a complete domain-separated registration plan. Interactive input defaults to no, structured input requires the exact plan identifier, and discovery is repeated before the unchanged candidate reaches the shared registration operation. The durable result enters existing guidance without promoting discovery hints into launch topology. Internal multi-attempt execution, workflow persistence, resume, and completion remain in parent issue #380. |
...
S142 is the next bounded child of #380. It preserves stored target selection as authoritative and reaches the existing bounded discovery composition only after a clean miss. One exact Steam application identifier or case-insensitive display name can produce a complete registration plan, which is flushed before default-no or exact structured confirmation and recomputed from fresh discovery before the shared registration operation. The resulting durable target enters S139-S141 without promoting executable hints into launch authority, so unresolved topology remains an explicit limitation rather than a fabricated session. Issue #398 closes with S142 while internal multi-attempt execution, workflow persistence, resume, and final completion remain in #380.
