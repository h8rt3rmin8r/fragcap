...
<Callout title="Current external path">
This is the v0.8 CLI. Deep Capture remains functional through external
mitmdump, while trust effects use native Windows APIs and production proxy replacement is tracked by
[#290](https://github.com/h8rt3rmin8r/fragcap/issues/290) under the complete
...
confirmation, an installed `mitmdump`, and compatible cold Steam launch facts.
...
| `--proxy-backend` | `mitmdump` | `mitmdump` | all | Select the local inspection proxy backend. |
