...
Command line interface for fragcap. Installs the `fragcap` binary.
...
The binary is named `fragcap`. It is one consumer of the `fragcap`
library crate, which carries the whole surface; a wrapper that needs to parse
this tool's output means a missing capability in the library.
