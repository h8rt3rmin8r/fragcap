...
`calibrate` resolves only an already registered target, obtains one complete query-only process image snapshot, reads retained compatibility facts, and consumes the proposal with child-environment routing and IPv4. Repeatable `--protocol` values use the closed concrete calibration vocabulary and are normalized before the proposal; they request measurement and never constitute evidence. Missing or non-positive routing evidence selects reachability first. Current exact positive routing permits at most the first useful S139 protocol step. The selected case delegates in-process to the existing low-level executor, including its complete plan authorization, deadlines, visible session trust when required, lifecycle, artifacts, terminal exit, cleanup, and append-only directly observed facts. One invocation never starts a second session.

Current exact positive routing with no candidate starts no session, reports protocol coverage unknown, and points to `fragcap deep-capture --id <ID> --local-db <EFFECTIVE_PATH> --launch`. Current positive facts for every supplied candidate report requested coverage complete without claiming universal coverage. A warm target starts no session by default and points to a `fragcap calibrate --id <ID> --local-db <EFFECTIVE_PATH> --restart-warm` command that preserves the candidate set; that explicit option reuses the existing operator-owned close-and-retry authority before any calibration plan. After a session, concrete final-client observations provide bounded sorted candidates, current facts are re-read, and S139 divides the combined set into completed and remaining work. A continuation carries one `--protocol` per remaining candidate with the same durable target and store binding. Paths are PowerShell-quoted so whitespace and interpolation characters cannot change the selected store. Changed, malformed, warm, unavailable, missing-observation, or otherwise non-ready authority cannot be reported as completed. Ambiguous, invalid, or unavailable proposals report every typed limitation before effects and invent no command. One additive `calibration.guidance` JSON event carries stable target identity, topology, action, status, launch cases, reason, images, limitations, requested, observed, completed, and remaining protocol arrays, plus nullable next command; human output projects the same facts.

On a stored selector miss, `calibrate` may discover and register one exact installed candidate. It emits `calibration.registration_plan` with the complete canonical plan, every conserved discovery count, and the warning count, followed by `calibration.registration` with the terminal registration decision, optional resulting stable target identifier, and whether proposal evaluation continued. The human plan renders the complete candidate, discovery account and warnings, and store before input. Structured input is exact and constant-time compared, and fresh discovery must reproduce all plan-bound authority before persistence while retaining exact changed no-match or ambiguity diagnostics. After registration, every downstream resolution replaces the discovery selector with the recovered stable identifier, so a numeric Steam application id cannot be reinterpreted as a listing row. A new discovery row carries no launch authority and can therefore produce the existing missing-declaration limitation without a session. The command does not run multiple attempts internally, persist workflow state, resume an interruption, promote discovery hints, or weaken ordinary Deep Capture eligibility or plan authorization.
...
Guided calibration tests additionally freeze repeatable candidate normalization, reachability-first ordering, one-session execution, final-client candidate eligibility, launcher and unknown exclusion, strongest same-session inspectability selection, exact fact reassessment, warm preservation, structured coverage arrays, and parseable durable continuation commands. Controlled reachability and TLS runs must prove that a requested protocol remains distinct from observed traffic and from the current exact fact that alone suppresses future work.
