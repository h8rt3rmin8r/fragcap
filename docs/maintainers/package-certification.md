...
The current primary artifacts are the versioned portable ZIP, versioned MSI, and standalone `catalog.db`. Each gets one `.sha256` sidecar. The ZIP and MSI carry the same six payload files. Checksum sidecars are not recursively checksummed, and the schema-version-3 certification report is CI evidence rather than another public release download.
