...
```text
fragcap calibrate ((<SELECTOR> | --target <SELECTOR> | --id <ID>) [--protocol <PROTOCOL>]... | --resume <WORKFLOW_ID>) [OPTIONS]
```

`calibrate` resolves or confirmation-gates one exact target, obtains complete query-only process image snapshots, reads retained compatibility facts, and consumes the proposal with child-environment routing and IPv4. Repeatable `--protocol` values use the closed concrete calibration vocabulary and are normalized before the proposal; they request measurement and never constitute evidence. Missing or non-positive routing evidence selects reachability first. After every successful case, concrete final-client observations may add bounded candidates, but only a fresh current positive fact permits progression. The command reopens the store, re-resolves the durable target, snapshots process state, and rebuilds S139 before every later attempt. Each selected case delegates independently to the existing low-level executor, including a fresh complete plan, separate confirmation, deadlines, visible session trust when required, lifecycle, artifacts, terminal exit, cleanup, and append-only directly observed facts.

Fresh calibration creates a workflow checkpoint after target registration and topology setup but before the first effectful attempt. `--resume` selects one exact positive identifier only from the effective local store and conflicts with target selectors and new protocol candidates. Resume compares the complete stored target authority with a fresh stable-id resolution, rereads processes and exact facts, and rebuilds S139. A changed target refuses the workflow before a new plan. A completed workflow resume is a no-effect current-status check. A row left in-flight by another process first becomes an `interrupted` pause and starts no effect. The next explicit resume reaches the existing recovery inspection before any new S134 plan.
...
Guided calibration tests additionally freeze repeatable candidate normalization, reachability-first ordering, deterministic multi-session execution, separate plan responses, exact case non-repetition across processes, the closed fourteen-attempt bound, distinct bundle destinations, final-client candidate eligibility, launcher and unknown exclusion, strongest same-session inspectability selection, exact fact reassessment before progression, every stop boundary, warm preservation, structured attempt and coverage fields, and parseable durable continuation commands. Store tests cover version 10 migration, checkpoint round trip, closed vocabulary and canonical-set refusal, bounded exact attempt history, target cascade, revision races, and unsupported record versions. Controlled command tests cover fresh identity, second-process resume, current-fact suppression, completed no-effect resume, in-flight interruption, target drift, every explicit pause, durable bundle ordinals, and distinct fresh plans. Controlled reachability and TLS runs must prove that a requested protocol remains distinct from observed traffic and from the current exact fact that alone suppresses future work.
