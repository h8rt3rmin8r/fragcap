...
- **The Deep Capture proxy ships with fragcap.** Do not configure a system-wide proxy; fragcap prepares its target-scoped native proxy session and manages its own purpose-specific CA only when the command explicitly includes `--trust-ca`.
...
Review the planned side effects before continuing: fragcap starts its native loopback proxy, starts ordinary packet Capture, prepares target-scoped proxy environment for the managed launch, may trust a purpose-specific CA in the current-user Root store when you authorize it with `--trust-ca`, writes a session bundle, and attempts to remove session-owned trust and tasks during cleanup. It never falls back to a system-wide proxy.

For a target that passed the eligibility check, run:

```powershell
fragcap deep-capture sample-target --launch --duration 5m --trust-ca --har
```
