...
The set of engines the provider recognizes.

- Variants: `Unreal`, `Unity`, `RenPy`.
- Derives: `Clone, Copy, Debug, PartialEq, Eq`.
- `as_str()` yields a stable label (`"unreal"`, `"unity"`, `"renpy"`) for
  diagnostics and tests. This label is not the provenance source; the provenance
  source is always the single string `engine-rule` (all engines share one
  provider and one provenance).
...
- `Resolved(EngineRuleTarget)` -- exactly one rule matched with exactly one
  candidate client.
...
- `ResolutionNotes` gains `engine_rule_ambiguous: Option<EngineRuleAmbiguity>`
  and a recorder `note_engine_rule_ambiguous(engine, candidates)`.
- `EngineRuleAmbiguity` is a small record `{ engine: Engine, candidates: usize }`
  (or an inline pair), enough to explain the decline.
- `Unresolved` carries the note through and exposes it via
  `engine_rule_ambiguous()`, alongside the existing `profile_not_found()`, so a
  caller that resolves nothing at all can report that the engine rule saw an
  ambiguous layout and stepped aside.
