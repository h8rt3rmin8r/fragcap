...
- `deep_capture.key_log_ready`: requested proxy-owned key log was created at its final bundle location and is available to live analyzers, with `session_id` and absolute `path`.
...
- `deep_capture.complete`: terminal session summary with `session_id`, `manifest`, `status`, `inspectable`, `metadata_only`, `unsupported`, and `cleanup_status`.
...
- Sensitive contents are never included. Paths may be included only for bundle artifacts and cleanup resources that are visible in the manifest or, for `deep_capture.key_log_ready`, will occupy that declared final bundle location when nonempty.
