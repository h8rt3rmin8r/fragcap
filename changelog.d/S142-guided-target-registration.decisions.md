...
- Discovery accepts only an exact Steam application identifier or case-insensitive display-name match. Ambiguity, partial names, executable hints, and path inference never select a target.
- Registration uses a complete, domain-separated plan that is flushed before default-no or exact structured confirmation. Fresh discovery must reproduce the plan before the existing shared registration operation runs.
