...
[Issue #372](https://github.com/h8rt3rmin8r/fragcap/issues/372) retains elevated first-run and immediate repeat measurements. Published v0.10.0 has coarse progress and a hidden `fragcap doctor --timings` switch that adds completion durations; it cannot update while a probe is blocked. S151's diagnostics below are unreleased and need a later authorized publication before installed testing. No measured historical stall cause or installed-host fix is claimed.
