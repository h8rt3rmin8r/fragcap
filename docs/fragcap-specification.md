...
The thirteen-topic `docs/audits/native-documentation-coverage.v2.json` contract is checked by `cargo xtask docs check` against current authored pages, exact required section headings, five closed example authorities and Cargo-discoverable non-ignored tests under explicit features. Missing, duplicate, unknown, reordered, historical-as-current, unsafe, absent, ignored or feature-inapplicable references fail. Published v0.10.1 identity is read from `docs/published-release.json`; later S154 through S157 source engineering remains a separate boundary. The contract proves documentation engineering traceability, not every prose statement or independent whole-product acceptance.

The stable reader-facing Native product contract route maps all thirteen topics and explains command-parser, manifest-reader, packet-golden, controlled-bundle and stable-API authority. It preserves independent review #333, installed QUIC retest #413, documentation reconciliation #331, final gate #334 and epic #278 as separate open outcomes. Later actual findings amend the bounded current contract before the parent can close; automation never invents them.
