...
On completion the installer offers to open the npcap download page, because npcap
is a separate install (see [Before you begin](#before-you-begin)). This is the same
npcap prerequisite, surfaced once more; if you already installed it with Wireshark
above, you can decline. The installer never downloads, bundles, or installs npcap
itself.
...
If a title you want is not listed (a non-Steam install, or one in an unusual
location), register it yourself. Point fragcap at the game's install folder:

```text
fragcap targets scan "C:\Games\MyGame"
```

or register an installed Steam title by its **App ID**, the number in the title's
Steam store or library URL (for example `306130` in
...
Run `fragcap targets` again afterward to see it in the numbered listing.
