...
| 0.1.76-draft | 2026-09-17 | W. Thompson | Corrects nondeterministic package loopback certification under #425. Updates sections 25.2, 27.3, and 28.1. Package report schema 4 uses exact structured proxy-start and same-session reached-client evidence as positive authority while preserving firewall containment, descendant ownership, sampled non-loopback rejection and cleanup. Socket-table presence remains diagnostic and negative evidence rather than a positive success prerequisite; schema 2 and schema 3 remain read-only. |
...
The eleven-topic `docs/audits/native-documentation-coverage.v1.json` inventory is checked by `cargo xtask docs check` against current authored pages and exact Cargo-discoverable non-ignored test authorities under explicit features. Missing, duplicate, historical, unsafe or absent references fail. The inventory proves engineering traceability, not every prose statement or independent whole-product acceptance. The reader-facing native documentation readiness reference states the separate published baseline, candidate changes and independent-review reconciliation boundary.
