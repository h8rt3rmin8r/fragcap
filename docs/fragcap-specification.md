...
The stable library proposal operation may prepare a later guided interface without adding another command in this revision. It consumes already-resolved target and process-image values supplied by its caller, reports strict topology and launch readiness, and emits an ordered exact reachability step or exact protocol steps with stable evidence reasons. It never performs the calibration, persists its proposal, or weakens the explicit invocation and plan-bound authorization requirements above.
